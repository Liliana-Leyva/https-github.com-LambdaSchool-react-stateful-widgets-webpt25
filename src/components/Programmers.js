...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers[programmersId].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: programmersId === null? 'royalblue' : 'gray', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setProgrammersId(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
